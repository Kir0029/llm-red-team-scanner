...
def create_provider(
    ...
    **kwargs: object,
    ...
    Args:
        model: Model name (e.g., 'qwen2.5:3b', 'nvidia/nemotron-3-ultra-550b-a55b:free')
               For OpenRouter: 'openrouter/google/gemini-2.0-flash-001'
               Or any model with '/' (auto-detected as OpenRouter)
        provider_type: Force provider type ('ollama', 'openai')
    ...
    if provider_type:
        ...
        if model.startswith("openrouter/"):
            ...
            # Any model with / (e.g. nvidia/nemotron, google/gemma, gpt-4/chatgpt)
            # is treated as OpenRouter (OpenAI-compatible API)
            pt = "openrouter"
        else:
            pt = "ollama"
